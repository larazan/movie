<section class="container mx-auto p-6 font-mono">
    <div class="w-full flex mb-4 p-2 justify-end">
        <x-jet-button wire:click="showCreateModal">Create Post</x-jet-button>
    </div>
                ...
                <tbody class="bg-white">
                    @foreach ($posts as $post)
                        <tr class="text-gray-700">
                            ...
                            <td class="px-4 py-3 text-ms font-semibold border">{{ $post->slug }}</td>
                            <td class="px-4 py-3 text-ms font-semibold border">{{ $post->body }}</td>

                            <td class="px-4 py-3 text-sm border">
                                <x-m-button wire:click="showEditModal({{ $post->id }})"
                                    class="bg-green-500 hover:bg-green-700 text-white">Edit</x-m-button>
                                <x-m-button wire:click="deletePost({{ $post->id }})"
    ...
    <x-jet-dialog-modal wire:model="showPostModal">
                            ...
                            <div class="px-4 py-5 bg-white sm:p-6">
                                <div class="flex flex-col space-y-3">
                                    <div class="col-start-1 sm:col-span-3">
                                        <label for="title" class="block text-sm font-medium text-gray-700">Post
                                            title
                                        </label>
                                    ...
                                    <div class="col-start-1 sm:col-span-3">
                                        <label for="body" class="block text-sm font-medium text-gray-700">Post
                                            body
                                        </label>
        ...
        <x-slot name="footer">
            <x-m-button wire:click="closePostModal" class="bg-gray-600 hover:bg-gray-800 text-white">Cancel</x-m-button>
            @if ($postId)
                <x-m-button wire:click="updatePost">Update</x-m-button>
            @else
                <x-m-button wire:click="createPost">Create</x-m-button>
            @endif
        </x-slot>
    </x-jet-dialog-modal>
</section>
