...
<x-loading-indicator />
    ...
    <div class="je jd jc ii">
        ...
        <!-- Left: Title -->
        <div class="ri _y">
            <h1 class="gu teu text-slate-800 font-bold">RateType ✨</h1>
        </div>
    ...
    <div class="bg-white bd rounded-sm border border-slate-200 rc">
        <header class="vc vu">
            <h2 class="gh text-slate-800">RateType Movies <span class="gq gp"></span></h2>
        </header>
                ...
                <table class="ux ou">
                    ...
                    <thead class="go gh gv text-slate-500 hp co cs border-slate-200">
                        <tr>
                            ...
                            <th class="vi wy w_ vo lm">
                                <div class="gh gt">Definition</div>
                            </th>
                            <th class="vi wy w_ vo lm">
                    ...
                    <tbody class="text-sm le lr">
                        ...
                        <tr>
                            ...
                            <td class="vi wy w_ vo lm">
                                <div class="gp text-slate-800 capitalize">{{ $rateType->name }}</div>
                            </td>
                            <td class="vi wy w_ vo lm">
                                <div class="gt">{{ $rateType->definition }}</div>
                            </td>
                            <td class="vi wy w_ vo lm">
                                @if ($rateType->status === 'inactive')
                                    <div class="inline-flex gp hf yl rounded-full gn vp vd">{{ $rateType->status }}</div>
                                @endif 

                                @if ($rateType->status === 'active')
                                    <div class="inline-flex gp hc ys rounded-full gn vp vd">{{ $rateType->status }}</div>
                                @endif 
                            </td>

                            <td class="vi wy w_ vo lm">
                                <div>{{ $rateType->created_at->format('d-m-Y') }}</div>
                            </td>

                            <td class="vi wy w_ vo lm of">
                                <div class="fm">
                                    <button class="gq xv rounded-full" wire:click="showEditModal({{ $rateType->id }})">
                                    <span class=" d">Edit</span>
                                        <svg class="os sf du" viewBox="0 0 32 32">
                                            <path d="M19.7 8.3c-.4-.4-1-.4-1.4 0l-10 10c-.2.2-.3.4-.3.7v4c0 .6.4 1 1 1h4c.3 0 .5-.1.7-.3l10-10c.4-.4.4-1 0-1.4l-4-4zM12.6 22H10v-2.6l6-6 2.6 2.6-6 6zm7.4-7.4L17.4 12l1.6-1.6 2.6 2.6-1.6 1.6z"></path>
                                        </svg>
                                    ...
                                    <button class="yl xy rounded-full" wire:click="deleteId({{ $rateType->id }})">
                                    <span class=" d">Delete</span>
                                        <svg class="os sf du" viewBox="0 0 32 32">
                                            <path d="M13 15h2v6h-2zM17 15h2v6h-2z"></path>
                                            <path d="M20 9c0-.6-.4-1-1-1h-6c-.6 0-1 .4-1 1v2H8v2h1v10c0 .6.4 1 1 1h12c.6 0 1-.4 1-1V13h1v-2h-4V9zm-6 1h4v1h-4v-1zm7 3v9H11v-9h10z"></path>
    ...
    <x-jet-dialog-modal wire:model="showRateTypeModal" class="">
        ...
        @if ($rateTypeId)
        <x-slot name="title" class="border-b">Update RateType</x-slot>
        @else
        <x-slot name="title" class="border-b bg-slate-200">
            <span class="font-semibold">Create RateType</span>
        </x-slot>
        @endif
                                ...
                                <div class="">
                                    <div class="flex flex-col space-y-3">
                                        <div class="col-start-1 sm:col-span-3">
                                            <label for="rateTypeName" class="block text-sm font-medium text-gray-700">
                                                Name
                                            </label>
                                            <input wire:model="rateTypeName" type="text" autocomplete="given-name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" />
                                            @error('rateTypeName')
                                                <div class="go re yl">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        <div class="col-start-1 sm:col-span-3">
                                            <label for="rateTypeDefinition" class="block text-sm font-medium text-gray-700">
                                                Definition
                                            </label>
                                            <input wire:model="rateTypeDefinition" type="text" autocomplete="given-name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" />
                                            @error('rateTypeDefinition')
                                                <div class="go re yl">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        <div class="col-start-1 sm:col-span-3">
                                            <label for="rateTypeDetail" class="block text-sm font-medium text-gray-700">
                                                Detail
                                            </label>
                                            <textarea wire:model="rateTypeDetail" autocomplete="given-name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" >{{ $rateTypeDetail }}</textarea>
                                            @error('rateTypeDetail')
                                                <div class="go re yl">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        <div class="col-span-6 sm:col-span-3">
                                            <label for="status" class="block text-sm font-medium text-gray-700">Status</label>
                                            <select wire:model="rateTypeStatus" class="h-full rounded-r border-t border-r border-b block appearance-none w-full bg-white border-gray-300 text-gray-700 py-2 px-4 pr-8 leading-tight focus:placeholder-gray-600 focus:text-gray-700 focus:outline-none">
        ...
        <x-slot name="footer">
            <div class="border-slate-200">
                <div class="flex flex-wrap justify-end fc">
                    <x-m-button wire:click="closeRateTypeModal" class="border-slate-200 hover:text-white hover--border-slate-300 g_">Cancel</x-m-button>
                    @if ($rateTypeId)
                    <x-m-button wire:click="updateRateType" class=" ho xi ye">Update</x-m-button>
                    @else
                    <x-m-button wire:click="createRateType" class=" ho xi ye2">Create</x-m-button>
                    @endif
                </div>
            </div>
