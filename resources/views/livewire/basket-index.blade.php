...
<x-loading-indicator />
    ...
    <div class="bg-white bd rounded-sm border border-slate-200 rc">
        <header class="vc vu">
            <h2 class="gh text-slate-800">Invoices <span class="gq gp">{{ $transactionCount }}</span></h2>
        </header>
                    ...
                    <tbody class="text-sm le lr">
                        ...
                        <tr>
                            ...
                            <td class="vi wy w_ vo lm">
                                <div class="gp text-slate-800">{{ $trans->id }}</div>
                            </td>
                            <td class="vi wy w_ vo lm">
                                <div class="gp text-slate-800">{{ $trans->description }}</div>
                            </td>
                            <td class="vi wy w_ vo lm">
                                <div class="gp text-slate-800">{{ number_format($trans->amount / 100, 2) }}</div>
                            </td>
                            <td class="vi wy w_ vo lm">
                                <div class="gp text-slate-800">{{ $trans->user->first_name }}</div>
                            </td>

                            <td class="vi wy w_ vo lm">
                                <div>{{ $trans->created_at->format('d-m-Y') }}</div>
                            </td>

                            <td class="vi wy w_ vo lm of">
                                <div class="fm">
                                    <button class="gq xv rounded-full" wire:click="showEditModal({{ $trans->id }})">
                                    ...
                                    <button class="yl xy rounded-full" wire:click="deleteId({{ $trans->id }})">
...
<x-jet-dialog-modal wire:model="showTransactionModal" class="">

@if ($transactionId)
<x-slot name="title" class="border-b">Update Transaction</x-slot>
@else
<x-slot name="title" class="border-b bg-slate-200">
    <span class="font-semibold">Create Transaction</span>
</x-slot>
@endif
                        ...
                        <div class="">
                            <div class="flex flex-col space-y-3">
                                <div class="col-start-1 sm:col-span-3">
                                    ...
                                    <input wire:model="amount" type="text" autocomplete="given-name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" />
                                    @error('amount')
                                ...
                                <div class="col-start-1 sm:col-span-3">
                                    ...
                                    <textarea wire:model="description" cols="50" autocomplete="given-name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" >{{ $description }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
...
<x-slot name="footer">
    <div class="border-slate-200">
        <div class="flex flex-wrap justify-end fc">
            <x-m-button wire:click="closeTransactionModal" class="border-slate-200 hover:text-white hover--border-slate-300 g_">Cancel</x-m-button>
            @if ($transactionId)
            <x-m-button wire:click="updateTransaction" class=" ho xi ye">Update</x-m-button>
            @else
            <x-m-button wire:click="createTransaction" class=" ho xi ye2">Create</x-m-button>
            @endif
        </div>
    </div>
