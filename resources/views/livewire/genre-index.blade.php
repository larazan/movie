...
<x-loading-indicator />
    ...
    <div class="je jd jc ii">
        ...
        <!-- Left: Title -->
        <div class="ri _y">
            <h1 class="gu teu text-slate-800 font-bold">Genre ✨</h1>
        </div>
        ...
        <div class="sn am jo az jp ft">
            ...
            <button class="btn ho xi ye" wire:click="showCreateModal">
                ...
                <span class="hidden trm nq">Create Genre</span>
            </button>
        </div>
    ...
    <div class="bg-white bd rounded-sm border border-slate-200 rc">
        <header class="vc vu">
            <h2 class="gh text-slate-800">Genre Movies <span class="gq gp"></span></h2>
        </header>
                    ...
                    <tbody class="text-sm le lr">
                        ...
                        <tr>
                            ...
                            <td class="vi wy w_ vo lm">
                                <div class="gp text-slate-800">{{ $genre->name }}</div>
                            </td>
                           
                            <td class="vi wy w_ vo lm">
                                @if ($genre->status === 'inactive')
                                    <div class="inline-flex gp hf yl rounded-full gn vp vd">{{ $genre->status }}</div>
                                @endif 

                                @if ($genre->status === 'active')
                                    <div class="inline-flex gp hc ys rounded-full gn vp vd">{{ $genre->status }}</div>
                                @endif 
                            </td>
                            ...
                            <td class="vi wy w_ vo lm of">
                                <div class="fm">
                                    <button class="gq xv rounded-full" wire:click="showEditModal({{ $genre->id }})">
                                    ...
                                    <button class="yl xy rounded-full" wire:click="deleteId({{ $genre->id }})">
    ...
    <x-jet-dialog-modal wire:model="showGenreModal" class="">

        @if ($genreId)
        <x-slot name="title" class="border-b">Update Genre</x-slot>
        @else
        <x-slot name="title" class="border-b bg-slate-200">
            <span class="font-semibold">Create Genre</span>
        </x-slot>
        @endif
                                ...
                                <div class="">
                                    <div class="flex flex-col space-y-3">
                                        ...
                                        <div class="col-span-6 sm:col-span-3">
                                            <label for="genreStatus" class="block text-sm font-medium text-gray-700">Status</label>
                                            <select wire:model="genreStatus" class="h-full rounded-r border-t border-r border-b block appearance-none w-full bg-white border-gray-300 text-gray-700 py-2 px-4 pr-8 leading-tight focus:placeholder-gray-600 focus:text-gray-700 focus:outline-none">
        ...
        <x-slot name="footer">
            <div class="border-slate-200">
                <div class="flex flex-wrap justify-end fc">
                    <x-m-button wire:click="closeGenreModal" class="border-slate-200 hover:text-white hover--border-slate-300 g_">Cancel</x-m-button>
                    @if ($genreId)
                    <x-m-button wire:click="updateGenre" class=" ho xi ye">Update</x-m-button>
                    @else
                    <x-m-button wire:click="createGenre" class=" ho xi ye2">Create</x-m-button>
                    @endif
                </div>
            </div>
